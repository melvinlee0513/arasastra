...
export function MainLayout({ children }: MainLayoutProps) {
  ...
  // Students use the light four-destination sidebar and their own page headers,
  // so the global streak/bell chrome is staff-only from here on.
  const studentShell = !!user && !isAdmin && !hasRole("tutor");
  const showTopChrome =
    !!user && !studentShell && !(isMobile && hidesMobileTopChrome(pathname));
  usePWAAnalytics();
      ...
      {showTopChrome && (
        <div
          className={`fixed top-0 right-0 z-30 flex items-center gap-2 p-3 ${
            isMobile ? "left-0 justify-end" : collapsed ? "left-16" : "left-64"
